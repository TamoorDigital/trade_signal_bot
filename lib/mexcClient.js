...
// Excludes tokenized-stock synthetic contracts (e.g. SAMSUNGSTOCK_USDT,
// SKHYNIXSTOCK_USDT, SPCXSTOCK_USDT) by default — a Feedback review on
// 2026-09-02 found these accounted for multiple immediate stop-outs, likely
// from off-session gaps and lower micro-structure fidelity than genuine
// crypto perpetuals. Add more exclusions via EXCLUDE_SYMBOLS (comma-separated
// exact symbols) or EXCLUDE_PATTERN (a regex, case-insensitive) in .env.
// This only affects the auto-selected top-volume watchlist — an explicit
// WATCHLIST env override is never filtered, since that's a deliberate choice.
const DEFAULT_EXCLUDE_PATTERN = /STOCK/i;

function isExcluded(symbol) {
  if (DEFAULT_EXCLUDE_PATTERN.test(symbol)) return true;
  const extraPattern = process.env.EXCLUDE_PATTERN;
  if (extraPattern && new RegExp(extraPattern, 'i').test(symbol)) return true;
  const extraList = (process.env.EXCLUDE_SYMBOLS || '').split(',').map(s => s.trim()).filter(Boolean);
  if (extraList.includes(symbol)) return true;
  return false;
}
...
async function fetchTopVolumeSymbols(n = 10) {
  ...
  return tickers
    .filter(t => t.symbol && t.symbol.endsWith('_USDT') && !isExcluded(t.symbol))
...
// Public contract spec (contractSize, min order size, step size) — needed to
// convert a USDT margin amount into the "vol" (number of contracts) the
// order-placement API expects. Cached briefly since these rarely change.
const contractDetailCache = new Map();
async function fetchContractDetail(symbol) {
  const cached = contractDetailCache.get(symbol);
  if (cached && Date.now() - cached.at < 10 * 60 * 1000) return cached.detail;
  const detail = await withRetry(async () => {
    const { data } = await axios.get(`${BASE}/detail`, { params: { symbol }, timeout: 10000 });
    if (!data || !data.success || !data.data) throw new Error(`MEXC contract detail fetch failed for ${symbol}: ${JSON.stringify(data)}`);
    return data.data;
  });
  contractDetailCache.set(symbol, { detail, at: Date.now() });
  return detail;
}

module.exports = { fetchKlines, fetchTopVolumeSymbols, fetchLastPrice, fetchContractDetail, throttledCall, withRetry };
