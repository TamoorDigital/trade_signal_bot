...
const { runAuditCheck } = require('./continuationAudit');
const { checkPendingConfirmations } = require('./pendingConfirmation');
const settings = require('./settings');
const { maybeRunLearningCycle } = require('./autoLearning');
...
async function scanSymbol(symbol, minScore) {
  ...
  // LONG_ONLY=1 in .env pauses shorts entirely. If the stronger-scoring side
  // this cycle is short, we skip the symbol rather than falling back to a
  // forced (likely weaker) long evaluation — a symbol whose best setup is
  // short just isn't traded this cycle, same as failing to clear MIN_SCORE.
  if (process.env.LONG_ONLY === '1' && candidate.direction === 'short') {
    log(`[scan] ${symbol}: skipped — LONG_ONLY=1 and the stronger setup this cycle was short (score ${candidate.score}/${candidate.maxScore})`);
    return;
  }
  ...
  const pending = {
    ...
    geminiBreakdown: gem.breakdown || null,
  ...
  store.addPendingTrade(pending);
  log(`[scan] ${symbol}: SIGNAL PENDING CONFIRMATION ${pending.direction.toUpperCase()} planned@${pending.plannedEntry} (score ${pending.ourScore}/${pending.maxScore}, gemini ${pending.geminiScore}/${pending.maxScore}) — waiting for price to confirm before opening any trade`);
}

async function scanOnce() {
  const minScore = settings.get().minScore;
  let watchlist;
  try {
...
// Without an IP whitelist, MEXC API keys expire after 90 days. If the user
// has told us when the key was created (MEXC_KEY_CREATED_DATE), warn in the
// Activity Log as expiry approaches, and again once it's likely expired,
// so a live-trading key doesn't silently stop working.
function checkKeyExpiry() {
  const createdStr = process.env.MEXC_KEY_CREATED_DATE;
  if (!createdStr) return;
  const created = new Date(createdStr);
  if (isNaN(created.getTime())) {
    log(`[mexc-key] MEXC_KEY_CREATED_DATE ("${createdStr}") isn't a valid date — use YYYY-MM-DD.`);
    return;
  }
  const daysElapsed = Math.floor((Date.now() - created.getTime()) / 86400000);
  const daysLeft = 90 - daysElapsed;
  if (daysLeft <= 0) {
    log(`[mexc-key] !!! Your MEXC API key was created ${daysElapsed} days ago and has likely EXPIRED (90-day limit, no IP whitelist). Auto-trade orders will fail until you generate a new key on MEXC and update MEXC_ACCESS_KEY/MEXC_SECRET_KEY (and MEXC_KEY_CREATED_DATE).`);
  } else if (daysLeft <= 15) {
    log(`[mexc-key] Reminder: your MEXC API key expires in ~${daysLeft} day(s) (90-day limit, no IP whitelist). Renew it soon on MEXC and update your env vars.`);
  }
}
...
function ensureTracking() {
  ...
  runAuditCheck(log);
  checkPendingConfirmations(log);
  maybeRunLearningCycle(log);
  state.trackTimer = setInterval(() => { trackOnce(log); runAuditCheck(log); checkPendingConfirmations(log); maybeRunLearningCycle(log); }, trackIntervalMs);
  checkKeyExpiry();
  setInterval(checkKeyExpiry, 24 * 60 * 60 * 1000); // re-check once a day
