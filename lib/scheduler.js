...
async function scanSymbol(symbol, minScore) {
  ...
  // Auto-trade: if enabled, place a real market order on MEXC right now.
  // If it fails, the trade still gets tracked (tracking-only) — we log the
  // failure loudly rather than silently dropping the signal.
  ...
  if (cfg.autoTradeEnabled) {
    try {
      ...
      if (order.wasClamped) {
        log(`[scan] ${symbol}: LIVE ORDER PLACED orderId=${order.orderId} vol=${order.vol} — leverage clamped to ${order.leverageUsed}x (requested ${order.leverageRequested}x, symbol max is ${order.leverageUsed}x) margin=${cfg.usdtPerTrade} USDT`);
      } else {
        log(`[scan] ${symbol}: LIVE ORDER PLACED orderId=${order.orderId} vol=${order.vol} leverage=${order.leverageUsed}x margin=${cfg.usdtPerTrade} USDT`);
      }
    } catch (err) {
      log(`[scan] ${symbol}: AUTO-TRADE FAILED (${err.message}) — signal still tracked, but NO live order was placed. Check your MEXC account/API keys.`);
    }
  }
